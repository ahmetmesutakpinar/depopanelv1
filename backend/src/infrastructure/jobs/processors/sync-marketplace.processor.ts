/**
 * Sync Marketplace Job Processor
 * 
 * Processes sync-marketplace background jobs.
 * 
 * Architecture:
...
import { SyncMarketplaceJobPayload, JobResult } from '../types.js';
import { container } from '../../di/index.js';
import { MarketplaceType } from '@prisma/client';
import { withJobContext } from '../../observability/job-context.js';

/**
 * Process Sync Marketplace Job
 * 
 * This processor:
 ...
 * @returns Job result
 */
export async function processSyncMarketplaceJob(
  // @ts-ignore - BullMQ Job type will be available after installation
  job: Job<SyncMarketplaceJobPayload>
): Promise<JobResult> {
  const {
    companyId,
    integrationId,
  ...
  return withJobContext(
    ...
    const useCase = container.getSyncMarketplaceUseCase(adapter);
