/**
 * Sync Stock Job Processor
 * 
 * Processes sync-stock background jobs.
 * 
 * Architecture:
...
import { SyncStockJobPayload, JobResult } from '../types.js';
import { container } from '../../di/index.js';
import { MarketplaceType } from '@prisma/client';
import { withJobContext } from '../../observability/job-context.js';

/**
 * Process Sync Stock Job
 * 
 * This processor:
 ...
 * @returns Job result
 */
export async function processSyncStockJob(
  // @ts-ignore - BullMQ Job type will be available after installation
  job: Job<SyncStockJobPayload>
): Promise<JobResult> {
  const { companyId, integrationId, productIds, warehouseId } = job.data;
  ...
  return withJobContext(
    ...
    const useCase = container.getSyncStockUseCase(adapter);
